...
(function () {
    ...
    document.addEventListener('click', function (e) {
        var inAllPanel = !!e.target.closest('.filter-all');
        document.querySelectorAll('details[open]').forEach(function (d) {
            if (d.contains(e.target)) return;
            /* The "All filters" panel lives outside its <details> toggle
               (as a sibling), so a click inside it must NOT close it. */
            if (d.classList.contains('filter--all') && inAllPanel) return;
    ...
    /* The "All filters" accordion lives outside ".filter-bar .filter"
       (its panel is a sibling), so wire its options + "Reset all" into
       the same chip / reshuffle system. */
    var allPanel = document.querySelector('.filter-all');
    if (allPanel) {
        var allInputs = allPanel.querySelectorAll('.filter__option input[type="checkbox"]');
        allInputs.forEach(function (input, oi) {
            var key = 'fa-' + oi;
            input.dataset.chipKey = key;
            inputByKey[key] = input;
            input.addEventListener('change', function () {
        ...
        allPanel.querySelectorAll('.filter__clear').forEach(function (clearBtn) {
            clearBtn.addEventListener('click', function () {
